      ...
      <nav aria-label="Hoofd" className={["mx-auto max-w-7xl px-4 sm:px-6", scrolled ? "py-2" : "py-4"].join(" ")}> 
        ...
        <div className="hidden md:grid grid-cols-2 grid-rows-2 gap-4 items-start w-full" style={{ minHeight: "8rem" }}>
          ...
          <div className="col-start-1 row-start-1 flex items-start min-w-0">
            <Link href="/" className="flex items-center gap-2 focus:outline-none focus-visible:ring-4 focus-visible:ring-[rgb(var(--dc-focus))] rounded-lg">
          ...
          <div className="col-start-2 row-start-1 flex items-center gap-3 justify-end min-w-0">
            <button
              ...
              onClick={() => setDark(d => !d)}
              className="flex h-10 w-10 items-center justify-center rounded-full border border-[rgb(var(--dc-border)/0.2)] text-[rgb(var(--dc-text))] transition hover:bg-[rgb(var(--dc-text)/0.06)] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[rgb(var(--dc-focus)/0.65)]"
            >
            ...
            <div role="group" aria-label="Taal switch" className="flex items-center gap-0">
              ...
              <input id="q" name="q" type="search" placeholder="Search" className="w-28 sm:w-44 rounded-full outline-none px-4 py-2 pr-9 min-w-0 text-fluid-sm" style={{ backgroundColor: 'rgb(var(--dc-text) / 0.06)', color: 'rgb(var(--dc-text))', border: '1px solid rgb(var(--dc-border) / 0.2)' }} onFocus={(e)=> { e.currentTarget.style.boxShadow = `0 0 0 4px rgb(var(--dc-focus) / 0.55)`; }} onBlur={(e)=> { e.currentTarget.style.boxShadow = ''; }} />
          ...
          <div className="col-start-1 row-start-2 flex items-end gap-4 flex-wrap sm:flex-nowrap">
            {MENUS.map(m => (
              <div key={m.label} className="relative">
                <button aria-expanded={openIndex === MENUS.indexOf(m)} aria-controls={`menu-${MENUS.indexOf(m)}`} onClick={() => setOpenIndex(openIndex === MENUS.indexOf(m) ? null : MENUS.indexOf(m))} className="inline-flex items-center gap-2 rounded-lg px-2 py-1 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[rgb(var(--dc-focus))] whitespace-nowrap dc-tooltip" aria-label={m.label} title={m.label}>
        ...
        <div className="md:hidden flex items-center justify-between">
          <button
            ...
            className="flex h-12 w-12 items-center justify-center rounded-full border border-[rgb(var(--dc-border)/0.2)] text-[rgb(var(--dc-text))] transition hover:bg-[rgb(var(--dc-text)/0.06)] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[rgb(var(--dc-focus)/0.65)]"
        ...
        <div id="mobile-menu" className={["md:hidden transition-all duration-300 overflow-hidden motion-reduce:transition-none", mobileOpen ? "max-h-[80vh] mt-3" : "max-h-0"].join(" ")}>
          <div className="rounded-2xl p-3 backdrop-blur" style={{ border: '1px solid rgb(var(--dc-border)/0.1)', backgroundColor: 'rgb(var(--dc-surface)/0.9)', color: 'rgb(var(--dc-text))' }}>
            ...
            <div className="mt-3 flex items-center gap-2">
              <button onClick={() => changeLanguage("nl")} aria-pressed={language === "nl"} className={["px-2 py-1 rounded-lg font-semibold focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[rgb(var(--dc-focus))] text-fluid-sm", language === "nl" ? "bg-[--color-brand] text-black" : "border border-white/30 text-white/90"].join(" ")}>NL</button>
              <button onClick={() => changeLanguage("en")} aria-pressed={language === "en"} className={["px-2 py-1 rounded-lg font-semibold focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[rgb(var(--dc-focus))] text-fluid-sm", language === "en" ? "bg-[--color-brand] text-black" : "border border-white/30 text-white/90"].join(" ")}>EN</button>
            ...
            <form role="search" className="mt-3">
              ...
              <input id="q-m" name="q" type="search" placeholder="Search" className="w-full rounded-full bg-white/10 text-white placeholder-white/70 outline-none px-4 py-2 border border-white/20 focus-visible:ring-4 focus-visible:ring-[rgb(var(--dc-focus))] text-fluid-sm" />
            </form>
          </div>
        </div>
      </nav>
