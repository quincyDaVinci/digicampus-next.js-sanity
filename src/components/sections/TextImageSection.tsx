...
export default function TextImageSection({ data }: TextImageSectionProps) {
  ...
  const imageUrl = hasValidImage ? urlForImage(image)?.width(900).fit('max').url() : null
      ...
      >
        ...
        {imageUrl ? (
          <div className="flex-1">
            <div className="relative aspect-[4/3] w-full overflow-hidden rounded-3xl border border-[rgb(var(--dc-border))] bg-[rgb(var(--dc-surface))] p-4 shadow-2xl">
              <Image
                src={imageUrl}
                alt={image?.alt || ''}
                fill
